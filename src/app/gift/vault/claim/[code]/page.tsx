...
export default function VaultGiftClaimPage({
  params,
}: {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/gift/vault/claim/${code}`);
        if (res.status === 404) {
          setNotFound(true);
          setLoading(false);
          return;
        ...
        <div className="rounded-2xl border border-cream-dark bg-white p-8 shadow-md">
          ...
          <div className="mt-6 rounded-lg border border-cream-dark bg-cream/50 p-4">
            ...
            <ul className="mt-3 space-y-1.5 text-sm text-navy">
              ...
              {summary.contents.video_credits > 0 && (
                <li>{summary.contents.video_credits} video message slots</li>
              )}
              {summary.contents.photo_credits > 0 && (
                <li>{summary.contents.photo_credits} photo upload slots</li>
              )}
              {summary.contents.audio_credits > 0 && (
                <li>{summary.contents.audio_credits} audio slots</li>
              )}
              <li>Sealed for up to 12 months &mdash; perfect for a 1st-anniversary reveal</li>
            </ul>
          </div>
            ...
            {loggedIn ? (
              <>
                ...
                <p className="mt-3 text-center text-xs text-warm-gray">
                  Signed in as{" "}
                  <span className="font-semibold text-navy">{userEmail}</span>.{" "}
                  <button
                    type="button"
