...
export default function CartPage() {
  ...
  function handleRemoveVault(id: string) {
    removeVaultFromCart(id);
    refreshCart();
  }
      ...
      <main className="min-h-screen bg-gradient-to-b from-cream to-cream-dark px-4 py-16 sm:py-24">
        <div className="mx-auto max-w-lg text-center">
          ...
          <p className="mt-3 text-warm-gray">
            Start adding gifts and messages to your cart.
          </p>
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-lg p-1.5 text-warm-gray-light transition hover:bg-red-50 hover:text-red-500"
                      aria-label="Remove vault credits"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="h-5 w-5">
                        <path fillRule="evenodd" d="M8.75 1A2.75 2.75 0 0 0 6 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 1 0 .23 1.482l.149-.022.841 10.518A2.75 2.75 0 0 0 7.596 19h4.807a2.75 2.75 0 0 0 2.742-2.53l.841-10.52.149.023a.75.75 0 0 0 .23-1.482A41.03 41.03 0 0 0 14 4.193V3.75A2.75 2.75 0 0 0 11.25 1h-2.5ZM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4ZM8.58 7.72a.75.75 0 0 0-1.5.06l.3 7.5a.75.75 0 1 0 1.5-.06l-.3-7.5Zm4.34.06a.75.75 0 1 0-1.5-.06l-.3 7.5a.75.75 0 1 0 1.5.06l.3-7.5Z" clipRule="evenodd" />
                      </svg>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-3">
                    ...
                    <div>
                      <span className="text-warm-gray-light">Total</span>
                      <p className="font-bold text-forest">${(vault.totalPrice / 100).toFixed(0)}</p>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-20 rounded-xl border border-cream-dark bg-white p-5 shadow-sm sm:p-6">
              ...
              <div className="mt-4 space-y-3">
                {letterItems.map((letter) => (
                  <div key={letter.id} className="flex items-center justify-between text-sm">
                    <span className="text-warm-gray truncate max-w-[60%]">
                      {letter.deliveryLabel} &times; {letter.quantity}
                    </span>
                ...
                {vaultItems.map((vault) => {
                  const parts: string[] = [];
                  if (vault.audioCredits > 0) parts.push(`${vault.audioCredits} audio`);
                  if (vault.videoCredits > 0) parts.push(`${vault.videoCredits} video`);
                  return (
                    <div key={vault.id} className="flex items-center justify-between text-sm">
                      <span className="text-warm-gray truncate max-w-[60%]">
                        Vault &mdash; {parts.join(" + ")}
                      </span>
                      <span className="font-medium text-navy">${(vault.totalPrice / 100).toFixed(0)}</span>
                    </div>
                  );
                })}
                {giftCreditItems.map((gc) => (
